...
export default function LeadManager() {
  ...
  const onSaveAll = async () => {
    try {
      ...
      const updates = leads
        .filter(
          (l) => tempStatuses[l.email] && tempStatuses[l.email] !== l.status
        )
  ...
  // apply sort
  let displayedRows = filtered.filter(
    (l) => !(isEditingAll && toDelete.has(l.email))
  );
  if (sortConfig.key) {
    ...
    <div style={S.page}>
      ...
      <div style={S.toolbar}>
        ...
        <select
          ...
          onChange={(e) =>
            setFilters((f) => ({ ...f, lead_source: e.target.value }))
          }
        >
        ...
        <select
          ...
          onChange={(e) =>
            setFilters((f) => ({ ...f, time_period: e.target.value }))
          }
        >
          ...
          <thead>
            <tr>
              ...
              <th style={S.th} onClick={() => requestSort('full_name')}>
                Name <span>{getSortIcon('full_name')}</span>
              </th>
              ...
              <th style={S.th} onClick={() => requestSort('submission_date')}>
                Submitted <span>{getSortIcon('submission_date')}</span>
              </th>
              <th style={S.th} onClick={() => requestSort('product_interest')}>
                Product <span>{getSortIcon('product_interest')}</span>
              </th>
              <th style={S.th} onClick={() => requestSort('lead_source')}>
                Source <span>{getSortIcon('lead_source')}</span>
              </th>
                    ...
                    {isEditingAll ? (
                      <>
                        <select
                          ...
                          onChange={(e) =>
                            onTempChange(l.email, e.target.value)
                          }
                          style={S.select}
                        >
                          {[
